...
const DeleteIntent = ({ id, intentName, setIsOpen, fetchIntentData }) => {
    const [conflictData, setConflictData] = useState(null);

    const handleClick = async (e) => {
        e.preventDefault();
        try {
            await axiosServer.delete(`/api/intent/${id}`);

            toast.success('Intent berhasil dihapus', {
                autoClose: 1000,
        ...
        <div className="warning_blurBG fade-in">
            <div className="warning_centered">
                <div className="warning_modal">
                    <div className="desc">
                        <p className="S2">Penghapusan Intent: {intentName}</p>
